...
export function AuthModal() {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenRes) => {
      try {
        ...
        const data = await post('/auth/google', { googleAccessToken: tokenRes.access_token });
  ...
  const Logo = () => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
      <div style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg, #7c3aed, #9333ea)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <span style={{ fontWeight: 600, fontSize: 15, color: '#1c1c1e' }}>Portfolio Doctor</span>
    </div>
  );
  ...
  const renderMain = () => (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <button
          onClick={() => { if (GOOGLE_CONFIGURED) googleLogin(); else setError('Google 로그인이 아직 설정되지 않았습니다'); }}
          disabled={loading}
  ...
  const renderEmailSignup = () => (
    <>
      ...
      <div style={{ background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.15)', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: '#7c3aed', lineHeight: 1.6 }}>
        Portfolio Doctor는 수익을 보장하는 서비스가 아닙니다.<br />분석 결과는 투자 참고 정보입니다.
      </div>
